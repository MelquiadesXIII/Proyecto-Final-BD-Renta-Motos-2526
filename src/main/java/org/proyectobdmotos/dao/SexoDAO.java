...
public class SexoDAO implements ISexoDAO {
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    ...
                    error = true;
                    Logger.logError("No se encontró sexo con nombre: " + nombre);
                }
            }
        } catch (SQLException e) {
            error = true;
            Logger.logError("Error al buscar id de sexo por nombre: " + e.getMessage());
        }

        if (error) {
            throw new RuntimeException("No se encontró sexo con nombre: " + nombre);
        }
    ...
    public String findNombreById(int id) {
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            error = true;
            Logger.logError("Error al buscar nombre de sexo por id: " + e.getMessage());
        }

        if (error) {
            throw new RuntimeException("No se encontró sexo con id: " + id);
        }
