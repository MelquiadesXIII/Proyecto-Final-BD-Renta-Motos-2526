...
AS $$
    ...
    WHERE m.id_moto NOT IN (
        -- Contrato activo ahora (empezado y no devuelto)
        SELECT co.id_moto
        FROM contrato co
        WHERE co.fecha_inicio <= CURRENT_DATE AND co.fecha_entrega IS NULL
        UNION
        -- Contrato que se solapa con el nuevo periodo
        SELECT co.id_moto
        FROM contrato co
        WHERE (co.fecha_inicio, co.fecha_fin) OVERLAPS (fecha_inicio, fecha_fin)
    )
